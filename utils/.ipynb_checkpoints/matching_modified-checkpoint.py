...
from .eval_metrics_modified import categorical_accuracy_continuous_tolerance_score
import numpy as np


def match_reconstruction_ground_truth_1(target_batch, reconstructed_batch, tolerance_map, return_indices=False,
                                      match_based_on='all'):
    """
    ...
    # print("match_recon Func reconstructed_batch: ",reconstructed_batch)
    ...
    for k, target_data_point in enumerate(target_batch):
        ...
        for l, recon_data_point in enumerate(reconstructed_batch):
            cost_all, cost_cat, cost_cont,gen,rac,man,female,white,black = categorical_accuracy_continuous_tolerance_score(
            ...
            final_gen_recstr.append(gen)
            final_rac_recstr.append(rac)
            final_male_recstr.append(man)
            final_female_recstr.append(female)
            final_white_recstr.append(white)
            final_black_recstr.append(black)

    # print(final_gen_recstr,":::::")
    # print(final_rac_recstr)
    ...
    if return_indices:
        print("ifff")
        return reordered_reconstructed_batch, batch_cost_all, batch_cost_cat, batch_cost_cont, row_ind, col_ind,final_gen_recstr,final_rac_recstr,final_male_recstr,final_female_recstr,final_white_recstr,final_black_recstr
    else:
        print("elsee")
        return reordered_reconstructed_batch, batch_cost_all, batch_cost_cat, batch_cost_cont,final_gen_recstr,final_rac_recstr,final_male_recstr,final_female_recstr,final_white_recstr,final_black_recstr
